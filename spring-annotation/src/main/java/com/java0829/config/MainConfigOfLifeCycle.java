 ...
 *      bean创建----> 初始化----> 销毁的过程
 * 容器管理bean的生命周期；
 * 我们可以自定义初始化和销毁方法，容器在bean进行到当前什么周期的时候来调用我们自定义的初始化和销毁方法
 *
 * 构造（对象创建）
 *      单实例：在容器创建的时候创建对象
 *      多实例：在每次获取的时候创建对象
 ...
 *  一但返回null，跳出for循环，不会执行后面的BeanPostProcessor.applyBeanPostProcessorsBeforeInitialization
 *
 ...
 * 3）、可以使用JSR250；
 *           @PostConstruct： 在bean创建完成且属性赋值完成，来执行初始化方法
 ...
 */
@ComponentScan("com.java0829.bean")
@Configuration
